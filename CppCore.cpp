...
double* SLAU(double** a, double x2) {
    ...
    for (int i = 0; i < 4; i += 2) {        
        ...
        ans[i + 1] = x2;
        j++;
        // ans[0] и ans[1] - решени€ при 1 корне = (1, 1) right
        // ans[2] и ans[3] - решени€ при 2 корне = (-1, 1) right
    }
    return ans;
}
...
Point GeoToPos(double B, double L){//переобразование гео координат в пр€моугольные. ѕараметры широта и долгота
	...
	L = L * M_PI / 180;
  	...
  	long double a = 6378245;//больша€ ось эллипсоида
...
Point Desperssion(PointPeople *arr, int N,Point expect){//дисперси€ xy
	Point desp;
	desp.x = 0;
	desp.y = 0;
...
int main (){
	...
	Point D = Desperssion(data,Data_Size,M);//дисперси€ 
	double Mxy = Expectationxy(data,Data_Size);//мат ожидание xy
	double r = (Mxy-M.x*M.y)/(sqrt(D.x)*sqrt(D.y));//коррел€ци€
	double alpha = atan((2*r*sqrt(D.x)*sqrt(D.y))/(D.x*D.x-D.y*D.y))/2;//угол поворота оси
	double ae = sqrt(2/(1/D.x+1/D.y-sqrt(pow((1/D.x-1/D.y),2)+4*(r/(sqrt(D.x)*sqrt(D.y))))*(r/(sqrt(D.x)*sqrt(D.y)))));//больша€ ось элипса рассеивани€
	double be = sqrt(2/(1/D.x+1/D.y+sqrt(pow((1/D.x-1/D.y),2)+4*(r/(sqrt(D.x)*sqrt(D.y))))*(r/(sqrt(D.x)*sqrt(D.y)))));//мала€ ось элипса рассеивани€
